...
then
  mongo oacis_development --eval 'db.hosts.insert({"status" : "enabled", "port" : 22, "max_num_jobs" : 10, "work_base_dir" : "~/oacis_work", "polling_interval" : 10, "min_mpi_procs" : 1, "max_mpi_procs" : 64, "min_omp_threads" : 1, "max_omp_threads" : 8, "name" : "K", "hostname" : "K", "user" : "${KUSER}"})'
fi
